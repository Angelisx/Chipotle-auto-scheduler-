...
def user_interface(calendar_list_summary):
    app = CTk()
    app.geometry("400x400")

    dropbox = CTkComboBox(master=app, values=[calendar_list_summary, calendar_list_summary, calendar_list_summary], fg_color="black", bg_color="white", dropdown_fg_color="red")
...
def List_caledar_ids():
    ...
    try:
        ...
        for calendar_list_entry in calendar_list['items']:
            print(f"Calendar: {calendar_list_entry['summary']}, ID: {calendar_list_entry['id']}")

            calendar_list_summary = calendar_list_entry['summary']
            user_interface(calendar_list_summary)
    except HttpError as error:
